...
router.get('/:id/animais', async (req, res) => {
  const [rows] = await pool.query(`
    ...
    FROM animais a WHERE a.lote_id = ? ORDER BY a.brinco
  `, [req.params.id])
  res.json(rows)
})
...
router.post('/', async (req, res) => {
  ...
  const [result] = await pool.query(
    'INSERT INTO lotes (nome, tipo, area_ha, capacidade) VALUES (?, ?, ?, ?)',
    [nome, tipo || 'pasto', area_ha, capacidade]
  )
...
router.put('/:id', async (req, res) => {
  const { nome, tipo, area_ha, capacidade } = req.body
  await pool.query(
    'UPDATE lotes SET nome=?, tipo=?, area_ha=?, capacidade=? WHERE id=?',
    [nome, tipo, area_ha, capacidade, req.params.id]
  )
  res.json({ ok: true })
})
